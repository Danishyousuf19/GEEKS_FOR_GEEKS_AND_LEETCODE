...
public class sfaest_path {
	class Solution {
		...
		public static int findShortestP(int[][] mat) {
			...
			for (int i = 0; i < n; i++)
				if (mat[i][0] == 1)
					mini = Math.min(mini, solveMe(i, 0, mat, n, m, vis));
			return mini == Integer.MAX_VALUE ? -1 : mini;
		}

		static int solveMe(int x, int y, int[][] mat, int n, int m, int[][] vis) {
			if (y == m - 1) {
				if (mat[x][y] == 1)
					return 0;
