...
public class goodComponents {
	class Solution {
		public int findNumberOfGoodComponent(int E, int V, int[][] edges) {
			...
			for (int i = 1; i <= V; i++) {
				if (!v[i]) { // if the current vertex is not visited
					int[] counts = dfs(adj, i, v); // perform depth-first search starting from i
		...
		private static int[] dfs(ArrayList<ArrayList<Integer>> A, int i, boolean v[]) {
			...
			counts[1] += A.get(i).size(); // add the number of edges connected to the current vertex

			for (int child : A.get(i)) { // iterate over the adjacent vertices of the current vertex
				if (!v[child]) { // if the child vertex is not visited
					int[] childCounts = dfs(A, child, v); // recursively perform depth-first search on the child vertex
